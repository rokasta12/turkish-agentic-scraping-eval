...
function formatNullableStatus(status: number | null) {
  return status === null ? 'status_unavailable' : String(status);
}
...
function renderSummary(records: DiscoveryRecord[]) {
  ...
  const fallbackFailedRecords = records.filter((r) => r.discovery.fetch_attempts.length > 1 && (!r.discovery.fetched_url || r.discovery.fetched_url === r.url)).length;
